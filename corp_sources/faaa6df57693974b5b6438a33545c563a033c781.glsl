...
void main()
{
  float x = (((true ? false : true) || false) ? ((false ? true : false) ? (0.5 + 0.5) : (true ? 2.0 : 0.5)) : +0.5);
  ...
  {
    vec4(1.0, 0.5, 1.0, (0.0 / ((-1.0 / 0.5) * -1.0)));
  }
  else
  {
    0.0;
  ...
  if (((x < (12.0 - 0.1)) || (x > (12.0 + 0.1))))
    rows = false;
  x = ((b[0][1] + b[1][1]) + b[2][1]);
  if (((!false || false) || (x > (15.0 + 0.5))))
    rows = ((false ? false : false) ? (false ? false : false) : true);
  x = ((b[0][2] + b[1][2]) + b[2][2]);
