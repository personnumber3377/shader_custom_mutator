...
vec4 gtf_Color = vec4(0.5, (0.5 - +(true ? 1.0 : 0.0)), ((false ? !false : true) ? 0.5 : 1.0), 0.0);
...
void main()
{
  ...
  vec3 sin_c = vec3(0.0);
  float fact;
  float fact_of = ((lerp(0.0, -1.0, 0.5) / -2.0) * lerp((-1.0 / 0.0), lerp(0.0, 0.5, 0.5), (0.0 - 0.0)));
  for (int i = 0; (i < 12); i++)
  {
    fact = 1.0;
    for (int j = 2; (j <= 23); j++)
  ...
  lerp(+(fact_of - fact_of), lerp(lerp(0.5, -1.0, 2.0), fact_of, lerp(-1.0, 0.5, fact_of)), fact_of);
}
