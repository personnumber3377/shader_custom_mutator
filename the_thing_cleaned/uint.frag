...
void main()
{
    int count = 1;
    uint u = t.y + 3u;
    const uint cu1error = 0xFFFFFFFF;
    const uint cu1 = 0xFFFFFFFFU;
    const uint cu2 = -1u;
    const uint cu3 = 1U;
    const uint cu4error = 1;
    const uint cu4 = 1u;
    if (cu1 == cu2)
        count *= 2;
    if (cu3 == cu4)
    ...
    if (cu2 == cu3)
        count *= 5;
    const uint cushiftediierror = 0xFFFFFFFF  >> 10;
    const  int  cshiftedii      = 0xFFFFFFFF  >> 10;
    const uint cushiftedui      = 0xFFFFFFFFu >> 10;
    const uint cushiftediuerror = 0xFFFFFFFF  >> 10u;
    ...
    if (cshiftedii == int(cushiftedui))
        count *= 13;
    uint shiftediierror = 0xFFFFFFFF  >> 10;
     int shiftedii      = 0xFFFFFFFF  >> 10;
    uint shiftedui      = 0xFFFFFFFFu >> 10;
    uint shiftediuerror = 0xFFFFFFFF  >> 10u;
