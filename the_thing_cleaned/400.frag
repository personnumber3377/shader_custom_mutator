...
void interp()
{
    ...
    interpolateAtCentroid(sampInArray);
    interpolateAtCentroid(sampInArray[2]);
    interpolateAtCentroid(sampInArray[2].xy);
    interpolateAtSample(sampInArray, 1);
    interpolateAtSample(sampInArray[i], 0);
    interpolateAtSample(s1.x, 2);
    interpolateAtSample(scalarIn, 1);
    interpolateAtOffset(sampInArray, vec2(0.2));
    interpolateAtOffset(sampInArray[2], vec2(0.2));
    interpolateAtOffset(sampInArray[2].xy, vec2(0.2));
    interpolateAtOffset(scalarIn + scalarIn, vec2(0.2));
    interpolateAtOffset(s2.x, vec2(0.2));
    float f;
    interpolateAtCentroid(f);
    interpolateAtSample(outp, 0);
}
...
void qlod()
{
    ...
    lod = textureQueryLod(sampBuf, pf);
    lod = textureQueryLod(sampRect, pf2);
}
uniform uint uu;
out uint iout;
void bitwiseConv()
