...
void testOffsets()
{
    ...
    sparseTextureGatherOffsetsARB(s2D, c2, offsets, texel);
    sparseTextureGatherOffsetsARB(is2DArray, c3, offsets, itexel, 2);
    sparseTextureGatherOffsetsARB(s2DRectShadow, c2, 2.0, offsets, texel);
}
...
layout(location = 12) in bName2 {
    float f;
    layout(location = 13) float g;
} bInst2[3];
layout(early_fragment_tests) in float f;
